...
int main()
{
    ...
    uint8_t* clamped = (uint8_t*) malloc(32*sizeof(uint8_t));
    clamp(clamped, c);

    uint8_t* d = (uint8_t *) malloc(32*sizeof(uint8_t));
    crypto_scalarmult(d, clamped, pubkey); //B^c
    printf("************* d = [B^c] *************\n");
    print_256(d);

    uint8_t* s = (uint8_t *) malloc(32 * sizeof(uint8_t));
    uint8_t* temp_da = (uint8_t *) malloc(32 * sizeof(uint8_t));
    uint8_t* temp_tc = (uint8_t *) malloc(32 * sizeof(uint8_t));

    crypto_core_ed25519_scalar_add(temp_da, d, a); //d+a
    crypto_core_ed25519_scalar_mul(temp_tc, t, c); //tc
    crypto_core_ed25519_scalar_add(s, temp_da, temp_tc); //s = d+a+tc

    clamp(clamped, pk);
    uint8_t* d_prime = (uint8_t *) malloc(32 * sizeof(uint8_t));
    crypto_scalarmult(d_prime, clamped, T); //T^b
    printf("************* d' *************\n");
    print_256(d_prime);

    uint8_t* S = (uint8_t *) malloc(32 * sizeof(uint8_t));
    crypto_scalarmult_base(S, s); //g^s

    printf("************* g^s *************\n");
    print_256(S);

    uint8_t* D_prime = (uint8_t *) malloc(32 * sizeof(uint8_t));
    crypto_scalarmult_base(D_prime, d_prime); // compute D'

    uint8_t* temp_AD = (uint8_t *) malloc(32 * sizeof(uint8_t));
    crypto_core_ed25519_scalar_mul(temp_AD, A, D_prime); //AD'

    uint8_t* temp_Tc = (uint8_t *) malloc(32 * sizeof(uint8_t));


    crypto_scalarmult(temp_Tc, c, T); //T^c

    uint8_t* check = (uint8_t *) malloc(32 * sizeof(uint8_t));
    crypto_core_ed25519_scalar_mul(check, temp_AD, temp_Tc); //AD'T^c

    printf("************* Check *************\n");
    print_256(check);

    int result;
    result = sodium_memcmp(S, check, sizeof(S));
    if(!result){
      printf("Match\n");
    }else{
      printf("Don't Match\n");
    ...
    printf("************* Schnorr *************\n");
    uint8_t* temp_ac = (uint8_t*) malloc(32*sizeof(uint8_t));
    crypto_core_ed25519_scalar_mul(temp_ac, a, c); //a*c
    crypto_core_ed25519_scalar_add(s, temp_ac, t); //s = a*c + t
    crypto_scalarmult_base(S, s); //g^s

    printf("************* S *************\n");
    print_256(S);

    crypto_scalarmult(temp_ac, c, A); //A^c
    crypto_core_ed25519_scalar_add(check, temp_ac, T); //c*A + T
...
void clamp(uint8_t* result, uint8_t* pk){
  memcpy(result, pk, sizeof(result));
  result[0] &= 248;
  result[31] &= 127;
  result[31] &= 64;
}
...
// Supporting functions pulled from library
